...
The data can come from external sensors connected to WiFi network by devices (e.g. ESP8266 devices family).
...
Try to run scripts for updating RRD databases and generating graphs:
```
...
Probably you will only need to add new items or show list of items. 
Below you can see examples.

### Insolation

This data can come from sensor based on photoresistor. 
You can easily save the state of the sun activity. It affects the state of the air inside. 
In a cloudy day these values will be significantly lower. 
